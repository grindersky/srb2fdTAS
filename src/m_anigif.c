...
static void GIF_bwrflush(void)
{
	...
	{
		WRITEBYTE(gifbwr_cur, (UINT8)(gifbwr_bits_buf&0xFF));
		++gifbwr_bufsize;
	}
...
static void GIF_headwrite(void)
{
	...
	WRITEUSHORT(p, rwidth);
	WRITEUSHORT(p, rheight);
...
static void GIF_framewrite(void)
{
	...
	// screen regions are handled in GIF_lzw
	{
		...
		WRITEBYTE(p, 0x2C);
		WRITEUSHORT(p, (UINT16)(blitx / scrbuf_downscaleamt));
		WRITEUSHORT(p, (UINT16)(blity / scrbuf_downscaleamt));
		WRITEUSHORT(p, (UINT16)(blitw / scrbuf_downscaleamt));
		WRITEUSHORT(p, (UINT16)(blith / scrbuf_downscaleamt));
		WRITEBYTE(p, 0); // no local table of colors
		...
		while (!gif_writeover)
		{
			...
			gifbwr_cur = gifbwr_buf;
			WRITEBYTE(p, gifbwr_bufsize);
			WRITEMEM(p, gifbwr_cur, gifbwr_bufsize);

			gifbwr_bufsize = 0;
			gifbwr_cur = gifbwr_buf;
		}
		WRITEBYTE(p, 0); //terminator
	}
